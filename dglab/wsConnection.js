...
const waveData = {
    ...
    // Bad: 渐强渐弱波形 (0% -> 25% -> 50% -> 75% -> 100% -> 75% -> 50% -> 30% -> 0%)
    "Bad": `["0A0A0A0A00000000","0A0A0A0A19191919","0A0A0A0A32323232","0A0A0A0A4B4B4B4B","0A0A0A0A64646464","0A0A0A0A4B4B4B4B","0A0A0A0A32323232","0A0A0A0A1E1E1E1E","0A0A0A0A00000000"]`,
    // Miss: 直接脉冲波形 (100% x6 -> 0% x1 -> 100% x2)
    "Miss": `["0A0A0A0A64646464","0A0A0A0A64646464","0A0A0A0A64646464","0A0A0A0A64646464","0A0A0A0A64646464","0A0A0A0A64646464","0A0A0A0A00000000","0A0A0A0A64646464","0A0A0A0A64646464","0A0A0A0A00000000","0A0A0A0A00000000","0A0A0A0A00000000"]`
}
...
function connectWs() {
    wsConn = new WebSocket("ws://110.42.63.208:9999/");
        ...
        switch (message.type) {
            case 'bind':
                if (!message.targetId) {
                    ...
                    qrcodeImg.makeCode("https://www.dungeon-lab.com/app-download.php#DGLAB-SOCKET#ws://110.42.63.208:9999/" + connectionId);
                }
                    ...
                    if (statusLightEl) {
                        statusLightEl.classList.remove("red");
                    }
                    if (statusBtnEl) {
                        statusBtnEl.innerText = "断开";
                        statusBtnEl.classList.add("red-background");
                    }
                    // 触发自定义事件通知live.html
                    window.dispatchEvent(new CustomEvent('dglab-connected', { detail: message }));
                    console.log("收到targetId: " + message.targetId + " msg: " + message.message);
                    if (typeof hideqrcode === 'function') {
                        hideqrcode();
                    }
                }
                else {
            ...
            case 'heartbeat':
                ...
                if (targetWSId !== '') {
                    ...
                    if (light) {
                        ...
                        setTimeout(() => {
                            if (light) light.style.color = '#ffe99d';
                        }, 1000);
                    }
                }
                break;
...
function handleAppMessage(message) {
    if (message.message.includes("strength")) {
        ...
        // 检查元素是否存在（兼容live.html）
        const channelAEl = document.getElementById("channel-a");
        const channelBEl = document.getElementById("channel-b");
        const softAEl = document.getElementById("soft-a");
        const softBEl = document.getElementById("soft-b");
        if (channelAEl) channelAEl.innerText = numbers[0];
        if (channelBEl) channelBEl.innerText = numbers[1];
        if (softAEl) softAEl.innerText = numbers[2];
        if (softBEl) softBEl.innerText = numbers[3];
